...
func RegisterRoutes(e *echo.Echo, app *App) {
	...
	protected.GET("/employees/me", app.HandleCurrentEmployee, app.RequireRoles(RoleAdmin, RoleEmployee))
	protected.GET("/employees", app.HandleListEmployees, app.RequireRoles(RoleAdmin))
	protected.POST("/employees", app.HandleCreateEmployee, app.RequireRoles(RoleAdmin))
	protected.PUT("/employees/:id", app.HandleUpdateEmployee, app.RequireRoles(RoleAdmin))
	protected.DELETE("/employees/:id", app.HandleDeleteEmployee, app.RequireRoles(RoleAdmin))
	...
	protected.PUT("/orders/:id/status", app.HandleUpdateOrderStatus, app.RequireRoles(RoleAdmin, RoleCustomer))
