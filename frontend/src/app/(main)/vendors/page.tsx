  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = await apiFetch<unknown>(
        `/api/vendors${params.size ? `?${params.toString()}` : ""}`
      );
  ...
  return (
    <RoleGuard allowedRoles={["admin"]}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-[var(--text-main)]">Vendors</h2>
          <p className="mt-1 text-sm text-[var(--text-muted)]">Manage supplier/vendor contacts.</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-[var(--border-main)]">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-[var(--text-muted)]">
                    {query.trim() ? "No vendors match your search." : "No vendors yet."}
                  </td>
                </tr>
              ) : (
                rows.map((r) => (
